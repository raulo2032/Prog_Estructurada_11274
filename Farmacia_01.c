/* farmacia_no_files.c
Sistema de administracion de farmacia totalmente en memoria (sin archivos en disco).

* Control de stock
* Registro de ventas (medicamentos OTC y con receta)
* Informes diarios y mensuales (generados y guardados EN MEMORIA)
* Reporte de stock critico (solo dueño)
* Borrar ventas al comenzar nuevo mes (solo dueño)
  Restricciones respetadas: no se usan structs ni typedef. Uso arrays y bucles for/while/switch.
  Compilar: gcc -o farmacia memoria.c
  */
...
int main()
{
    ...
    while (running)
    {
        printf("\n==== SISTEMA DE GESTION - FARMACIA (MEMORIA) ====\n");
...
/* Genera informe mensual por días y lo guarda EN MEMORIA */
void report_monthly_and_store()
{
    if (sales_count == 0)
    {
    ...
    /* Construir texto del informe y guardarlo en memoria */
    ...
    /* llenar buffer */
    char buf[REPORT_TEXT_LEN];
    int pos = 0;
    pos += snprintf(buf + pos, REPORT_TEXT_LEN - pos, "Informe Mensual %02d/%04d\n", month, year);
    pos += snprintf(buf + pos, REPORT_TEXT_LEN - pos, "Dia | CantVentas | Total\n");
    for (d = 1; d <= MAX_DAYS; d++)
    {
        if (day_counts[d] > 0)
        {
            pos += snprintf(buf + pos, REPORT_TEXT_LEN - pos, "%2d  | %10d | %8.2f\n", d, day_counts[d], day_totals[d]);
        }
        if (pos >= REPORT_TEXT_LEN - 200)
            break; /* evitar overflow */
    }
    pos += snprintf(buf + pos, REPORT_TEXT_LEN - pos, "TOTAL_MENSUAL | %.2f\n", total_month);

    /* Guardar en arrays paralelos */
    report_month[report_count] = month;
    report_year[report_count] = year;
    strncpy(report_text[report_count], buf, REPORT_TEXT_LEN - 1);
...
/* Mostrar lista de informes guardados en memoria */
void show_stored_reports()
{
    int i;
    if (report_count == 0)
...
/* Ver informe guardado (por indice) */
void view_report()
{
    int idx;
    if (report_count == 0)
